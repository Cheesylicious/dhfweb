...
from sqlalchemy.orm import joinedload  # NEU: joinedload importieren
...
# --- 5. Datenbank-Modelle (ShiftType angepasst) ---
class Role(db.Model):
    # (Unverändert)
...
class Shift(db.Model):
    ...
    shift_type = db.relationship('ShiftType')  # (Kein lazy='joined')
    __table_args__ = (db.UniqueConstraint('user_id', 'date', name='_user_date_uc'),)

    def to_dict(self):
        # (Sicherstellen, dass shift_type geladen ist, bevor to_dict aufgerufen wird)
        abbr = "ERR"  # (Sollte nicht passieren, wenn get_shifts joinedload verwendet)
        if self.shift_type:
...
# --- HILFSFUNKTION (KORRIGIERT für Spillover) ---
def _calculate_user_total_hours(user_id, year, month):
    """
    Berechnet die Gesamtstunden für EINEN User in einem Monat.
    Berücksichtigt jetzt auch Stunden-Übertrag (spillover) vom Vormonat.
    """

    # 1. Finde den letzten Tag des Vormonats
    try:
        first_day_of_current_month = datetime(year, month, 1)
        last_day_of_previous_month = first_day_of_current_month - timedelta(days=1)
    except ValueError:
        app.logger.error(f"Ungültiges Datum in _calculate_user_total_hours: {year}-{month}")
        return 0.0

    # --- KORREKTUR (Regel 2: Innovativ, keine langsamen Abfragen) ---
    # Wir holen die Schichten und rechnen in Python statt in SQL

    total_hours = 0.0

    # 2. Query: Normale Stunden, die in diesem Monat eingetragen sind
    # (WICHTIG: .options(db.joinedload(Shift.shift_type)) lädt die Beziehung explizit)
    ...
    for shift in shifts_in_this_month:
        # Python-Check (Regel 1: Keine Funktion kaputt machen)
        if shift.shift_type and shift.shift_type.is_work_shift:
            total_hours += shift.shift_type.hours

            # 3. Query: Übertrag-Stunden (spillover) vom Vormonat
...
# --- ENDE HILFSFUNKTION ANPASSUNG ---


# --- 10a. API für Schicht-Typen (Angepasst) ---
...
def update_shifttype(type_id):
    ...
    st.hours_spillover = data.get('hours_spillover', st.hours_spillover)  # (Der Speicher-Fix)
    # --- ENDE KORREKTUR ---
...
def get_shifts():
    ...
    # --- KORREKTUR: Explizites Laden von shift_type (lazy loading) ---
    ...
    # Abfrage 2: Berechne Gesamtstunden (Ruft die NEUE Hilfsfunktion auf)

    first_day = datetime(year, month, 1)
    last_day_prev_month = first_day - timedelta(days=1)

    # Finde alle relevanten User-IDs
    ...
    # Kombinierte, einzigartige Liste von User-IDs
    relevant_user_ids = {uid[0] for uid in user_ids_this_month.union(user_ids_prev_month)}

    totals_dict = {}
    for user_id in relevant_user_ids:
        # Verwende die neue, korrekte Berechnungslogik
...
# --- 10c. API für Schichtplan (POST-Anfrage KORRIGIERT) ---
...
def save_shift():
    ...
    response_data = {}
    status_code = 200  # (Standard ist OK)

    # --- KORREKTUR: Commit-Logik (Regel 1) ---
    if existing_shift:
        if shifttype_id == free_type_id:
            ...
            # (Commit wird nach außen verschoben)
        ...
        if shifttype_id != free_type_id:
            ...
            status_code = 201  # (Nur bei NEUerstellung)
        else:
            response_data = {"message": "Keine Aktion (bereits Frei)"}

    # EINMALIGER Commit für ALLE Operationen (Delete, Update, Create)
    try:
        db.session.commit()
    except Exception as e:
        db.session.rollback()
        app.logger.error(f"Fehler beim Speichern der Schicht: {str(e)}")
        return jsonify({"message": f"Datenbankfehler: {str(e)}"}), 500
    # --- ENDE KORREKTUR ---

    # (Lade die Abkürzung neu, falls sie geändert wurde - wichtig für 'to_dict')
    if 'id' in response_data:
        # (Sicherstellen, dass die Beziehung für to_dict geladen ist)
        reloaded_shift = db.session.get(Shift, response_data['id'])
        if reloaded_shift:
    ...
    # Prüfe, ob wir auch den Folgemonat aktualisieren müssen
    try:
        next_day = shift_date + timedelta(days=1)
        if next_day.month != shift_date.month:
            # Ja, diese Schicht war am Monatsende. Berechne den Folgemonat neu.
        ...
        app.logger.warning(f"Konnte Folgemonat nicht berechnen: {str(e)}")

    return jsonify(response_data), status_code


# --- 11. Start-Logik (Angepasst) ---
...
def create_default_shifttypes():
    # --- ANGEPASST (auf Basis Ihrer Originaldatei) ---
    # Annahme: N. (Nacht) ist 22:00 - 06:00 (8h) -> 2h am Tag 1, 6h am Tag 2 (Spillover)
    # T. (Tag) ist 8h
    # 6 (Kurz) ist 6h
    ...
    default_types = [
        ...
        {'name': 'Nacht (N)', 'abbreviation': 'N.', 'color': '#5D6D7E',
         'hours': 2.0, 'hours_spillover': 6.0, 'is_work_shift': True},  # 8h gesamt -> 2h + 6h
    ...
    # --- (Logik zum Einfügen/Aktualisieren) ---
    for st_data in default_types:
        # (Wir verwenden joinedload, um den Fehler beim Start zu vermeiden)
